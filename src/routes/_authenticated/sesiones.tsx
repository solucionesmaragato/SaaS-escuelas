...
import { isDireccionRole, isProfesorRole } from "@/lib/tenantQuery";
...
function SesionesCalendarioPage() {
  ...
  const { list } = useSesiones(dateRange, filterCenterId);

  const sesiones = list.data?.sesiones ?? [];
  const tenantFilters = list.data?.filters ?? {
    uniqueAlumnos: [],
    uniqueProfesores: [],
    uniqueAulas: [],
  ...
  const [FILTRO_ALUMNO, setFILTRO_ALUMNO] = useState("");
  const [verLeads, setVerLeads] = useState(true);
  ...
  const [verClasesNormales, setVerClasesNormales] = useState(
    () => isProfesorRole(rol) || isDireccionRole(rol),
  );
  ...
  const cascadingOptions = useMemo(() => {
    ...
    const poolEspecialidad = sesiones.filter((b) =>
  ...
  const renderCompactMatriculaBlock = (ev: GroupedSession) => (
    ...
    >
      ...
      <span className="shrink-0 text-[9px] opacity-70 max-w-[38%] truncate">
        {ev.TEXTO_AULA}
      </span>
    </button>
  );
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-4">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Calendario de Sesiones</h1>
          <p className="text-sm text-muted-foreground">
            Vista inteligente de clases, leads e incidencias
          </p>
        </div>
        ...
        <div className="lg:col-span-3 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <Button
              ...
              className={
                verClasesNormales
                  ? "bg-blue-100 text-blue-900 border-blue-300 hover:bg-blue-200 shadow-sm ring-1 ring-blue-300/60"
                  : "border-blue-200 text-blue-800 hover:bg-blue-50 opacity-60"
              }
            ...
            <Button
              ...
              className={
                verLeads
                  ? "bg-amber-100 text-amber-900 border-amber-300 hover:bg-amber-200 shadow-sm ring-1 ring-amber-300/60"
                  : "border-amber-200 text-amber-800 hover:bg-amber-50 opacity-60"
              }
            ...
            <Button
              ...
              className={
                verIncidencias
                  ? "bg-red-100 text-red-900 border-red-300 hover:bg-red-200 shadow-sm ring-1 ring-red-300/60"
                  : "border-red-200 text-red-800 hover:bg-red-50 opacity-60"
              }
          ...
          <Card className="p-3 shadow-sm">
            {calendarView !== "day" && (
              <div className="grid grid-cols-7 text-center font-medium text-xs text-muted-foreground border-b pb-2 mb-1">
                <div>Lun</div>
                <div>Mar</div>
                <div>Mié</div>
                <div>Jue</div>
            ...
            {calendarView === "month" && (
              <div className="grid grid-cols-7 gap-px bg-border/60 rounded-lg overflow-hidden min-h-[560px]">
                {list.isLoading ? (
                  Array.from({ length: 35 }).map((_, i) => (
                    <Skeleton key={i} className="w-full h-full min-h-[95px] rounded-none" />
                  ))
                ) : (
                  diasDelMes.map((dia, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`bg-background p-1.5 flex flex-col min-h-[95px] ${
                          !dia.isCurrentMonth ? "opacity-40 bg-muted/30" : ""
                        }`}
                      >
                        <span
                          className={`text-[11px] font-semibold self-end w-6 h-6 flex items-center justify-center rounded-full ${
                            isToday
                        ...
                        <div className="space-y-1 mt-1 flex-1 overflow-y-auto max-h-[78px]">
                          {eventosDelDia.slice(0, 4).map((ev) => renderEventButton(ev))}
                          {eventosDelDia.length > 4 && (
                            <button
                              type="button"
                              onClick={() => navigateToDay(dia.date)}
                              className="w-full text-[9px] font-medium text-center text-primary bg-muted/50 hover:bg-muted rounded-md py-0.5 cursor-pointer transition-colors"
                            >
            ...
            {calendarView === "week" && (
              <div className="grid grid-cols-7 gap-px bg-border/60 rounded-lg overflow-hidden min-h-[360px]">
                {list.isLoading ? (
                  Array.from({ length: 7 }).map((_, i) => (
                    <Skeleton key={i} className="w-full h-full min-h-[90px] rounded-none" />
                  ))
                ) : (
                  diasDeLaSemana.map((dia, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className="bg-background p-1 flex flex-col min-h-[90px]"
                      >
                        <span
                          className={`text-[10px] font-semibold self-end w-5 h-5 flex items-center justify-center rounded-full ${
                            isToday
                              ? "bg-primary text-primary-foreground"
                              : "text-muted-foreground"
                          }`}
            ...
            {calendarView === "day" && (
              <div className="rounded-lg border overflow-hidden min-h-[360px]">
                {list.isLoading ? (
      ...
      <Dialog open={!!selectedEvent} onOpenChange={(o) => !o && setSelectedEvent(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedEvent?.FECHA_EXACTA?.split("T")[0]} ·{" "}
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                  ...
                  <p className="font-semibold">{selectedEvent.TEXTO_PROFESOR}</p>
                </div>
                <div>
                  <p className="text-muted-foreground mb-0.5">Aula</p>
                  <p className="font-semibold">{selectedEvent.TEXTO_AULA}</p>
                </div>
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-2 max-h-[60vh] overflow-y-auto pr-2">
                  {alumnosOrdenados.map((alumno, idx) => (
                    ...
                    >
                      <span className="font-medium text-sm">{alumno.TEXTO_ALUMNO}</span>
              ...
              {selectedEvent.COLOR_INCIDENCIA === "rojo" && (
                <div className="p-3 rounded-md border text-xs bg-red-50 border-red-200 text-red-900">
