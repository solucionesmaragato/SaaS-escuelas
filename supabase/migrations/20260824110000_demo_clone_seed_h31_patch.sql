-- H3.1: leaner demo Madrid seed (no mandatos, no seed PII, skip heavy tables)
...
BEGIN
  ...
  SELECT DISTINCT ON (alum_m.new_id, gh_m.new_id)
    hm_m.new_id,
    p_target_cliente,
    mat_m.new_id,
    alum_m.new_id,
  ...
  WHERE hm."ID_CLIENTE" = p_source_cliente
    ...
    AND gh_m.new_id IS NOT NULL
  ORDER BY alum_m.new_id, gh_m.new_id, hm."ID_HORARIO"
  ...
  UPDATE public."GRUPOS_HORARIOS" tgt
  SET "ID_PROFESOR" = pm.new_id,
      "ID_AULA" = am.new_id
  FROM public."GRUPOS_HORARIOS" src
  JOIN demo_id_map gm ON gm.entity = 'grupo_horario' AND gm.old_id = src."ID_GRUPO_HORARIO"
  LEFT JOIN demo_id_map pm ON pm.entity = 'profesor' AND pm.old_id = src."ID_PROFESOR"
  LEFT JOIN demo_id_map am ON am.entity = 'aula' AND am.old_id = src."ID_AULA"
  WHERE tgt."ID_GRUPO_HORARIO" = gm.new_id
    AND tgt."ID_CLIENTE" = p_target_cliente
    AND src."ID_CLIENTE" = p_source_cliente;

  UPDATE public."HORARIOS_MATRICULAS" tgt
  SET "ESTADO" = src."ESTADO"
  FROM public."HORARIOS_MATRICULAS" src
  JOIN demo_id_map hm ON hm.entity = 'horario' AND hm.old_id = src."ID_HORARIO"
  WHERE tgt."ID_HORARIO" = hm.new_id
    AND tgt."ID_CLIENTE" = p_target_cliente
    AND src."ID_CLIENTE" = p_source_cliente;
