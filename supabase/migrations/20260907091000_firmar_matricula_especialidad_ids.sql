-- Fix FK MATRICULAS.ESPECIALIDAD: usar primer ID de ESPECIALIDADES_IDS, no labels unidos.
-- Basado en 20260907082000 (firmar + TEXTOS_LEGALES + estado Activo).
...
DECLARE
  ...
  v_id_curso text;
  v_especialidad text;
  v_especialidad_matricula text;
  v_especialidad_payload text;
  v_especialidad_id_candidato text;
  v_pdf_url text;
  v_hash text;
...
BEGIN
  ...
  INTO v_id_curso, v_especialidad_matricula
  ...
  v_especialidad_payload := NULL;
  v_especialidad_id_candidato := NULLIF(btrim(p_datos->'ESPECIALIDADES_IDS'->>0), '');
  IF v_especialidad_id_candidato IS NOT NULL THEN
    SELECT e."ID_ESPECIALIDAD"
    INTO v_especialidad_payload
    FROM public."ESPECIALIDADES" e
    WHERE e."ID_ESPECIALIDAD" = v_especialidad_id_candidato
      AND e."ID_CLIENTE" = v_sol."ID_CLIENTE"
    LIMIT 1;
  END IF;

  IF v_especialidad_payload IS NULL THEN
    v_especialidad_id_candidato := NULLIF(btrim(p_datos->>'ESPECIALIDAD'), '');
    IF v_especialidad_id_candidato IS NOT NULL
       AND position(',' in v_especialidad_id_candidato) = 0 THEN
      SELECT e."ID_ESPECIALIDAD"
      INTO v_especialidad_payload
      FROM public."ESPECIALIDADES" e
      WHERE e."ID_ESPECIALIDAD" = v_especialidad_id_candidato
