-- Guardado de LINK_RECIBOS_ZIP tras empaquetar PDFs borrador del lote.

CREATE OR REPLACE FUNCTION public.guardar_link_zip_remesa(
    p_id_cliente text,
    p_id_centro text,
    p_id_curso text,
    p_mes_periodo text,
...
BEGIN
    ...
    SET "LINK_RECIBOS_ZIP" = NULLIF(trim(p_link), '')
...
GRANT EXECUTE ON FUNCTION public.guardar_link_zip_remesa(text, text, text, text, text) TO authenticated;
