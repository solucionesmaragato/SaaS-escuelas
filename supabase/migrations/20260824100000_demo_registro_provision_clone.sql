-- Demo public signup: sequence, Madrid clone from ESC_018, tenant provisioning.

CREATE SEQUENCE IF NOT EXISTS public.demo_cliente_seq START 1;

CREATE OR REPLACE FUNCTION public.next_demo_cliente_id()
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_next bigint;
BEGIN
  SELECT COALESCE(MAX(CAST(substring("ID_CLIENTE" FROM 6) AS integer)), 0)
  INTO v_max
  FROM public."CLIENTES"
  WHERE "ID_CLIENTE" ~ '^DEMO-[0-9]+$';

  PERFORM setval(
    'public.demo_cliente_seq',
    GREATEST(v_max, (SELECT last_value FROM public.demo_cliente_seq)),
    true
  );
  v_next := nextval('public.demo_cliente_seq');
  RETURN 'DEMO-' || lpad(v_next::text, 4, '0');
END;
$$;
...
BEGIN
  ...
  INSERT INTO demo_id_map (entity, old_id, new_id)
  SELECT 'sesion', s."ID_SESION",
         'SES_' || gen_random_uuid()::text || '_' || to_char(s."FECHA_EXACTA", 'YYYYMMDD')
  FROM public."SESIONES" s
  WHERE s."ID_CLIENTE" = p_source_cliente AND s."ID_CENTRO" = p_source_centro;

  INSERT INTO public."SESIONES" (
    "ID_SESION", "ID_CLIENTE", "ID_MATRICULA", "ID_HORARIO", "ID_ALUMNO", "FECHA_EXACTA",
    "HORA_INICIO", "HORA_FIN", "ID_PROFESOR", "ID_AULA", "ESPECIALIDAD", "ESTADO", "NOTAS",
    "TITULO_CALENDARIO", "ID_GRUPO_HORARIO", "ID_CENTRO", "ID_CURSO", "FECHA_CREACION", "ULTIMA_MODIFICACION"
  )
  SELECT
    sm.new_id,
    p_target_cliente,
  ...
  FROM public."SESIONES" s
  JOIN demo_id_map sm ON sm.entity = 'sesion' AND sm.old_id = s."ID_SESION"
  LEFT JOIN demo_id_map mat_m ON mat_m.entity = 'matricula' AND mat_m.old_id = s."ID_MATRICULA"
  LEFT JOIN demo_id_map hor_m ON hor_m.entity = 'horario' AND hor_m.old_id = s."ID_HORARIO"
  LEFT JOIN demo_id_map alum_m ON alum_m.entity = 'alumno' AND alum_m.old_id = s."ID_ALUMNO"
  LEFT JOIN demo_id_map pro_m ON pro_m.entity = 'profesor' AND pro_m.old_id = s."ID_PROFESOR"
  LEFT JOIN demo_id_map aul_m ON aul_m.entity = 'aula' AND aul_m.old_id = s."ID_AULA"
  LEFT JOIN demo_id_map esp_m ON esp_m.entity = 'especialidad' AND esp_m.old_id = s."ESPECIALIDAD"
  LEFT JOIN demo_id_map gh_m ON gh_m.entity = 'grupo_horario' AND gh_m.old_id = s."ID_GRUPO_HORARIO"
  LEFT JOIN demo_id_map cur_m ON cur_m.entity = 'curso' AND cur_m.old_id = s."ID_CURSO"
  WHERE s."ID_CLIENTE" = p_source_cliente AND s."ID_CENTRO" = p_source_centro;

  GET DIAGNOSTICS v_n = ROW_COUNT;
  v_counts := v_counts || jsonb_build_object('SESIONES', v_n);
  ...
  INSERT INTO demo_id_map (entity, old_id, new_id)
  SELECT 'incidencia', i."ID_INCIDENCIA", public.demo_gen_prefixed('INC')
  FROM public."INCIDENCIAS" i
  WHERE i."ID_CLIENTE" = p_source_cliente AND i."ID_CENTRO" = p_source_centro;

  INSERT INTO public."INCIDENCIAS" (
    "ID_INCIDENCIA", "ID_CLIENTE", "ID_MATRICULA", "ID_HORARIO", "ID_ALUMNO", "ID_SESION",
    "TIPO_INCIDENCIA", "TIPO_FALTA", "NOTAS", "ID_PROFESOR", "FECHA_EXACTA", "HORA_INICIO",
    "HORA_FIN", "ESTADO_CONSULTA", "ID_ESPECIALIDAD", "ID_CENTRO", "ID_CURSO", "ID_AULA",
  ...
  FROM public."INCIDENCIAS" i
  JOIN demo_id_map im ON im.entity = 'incidencia' AND im.old_id = i."ID_INCIDENCIA"
  LEFT JOIN demo_id_map mat_m ON mat_m.entity = 'matricula' AND mat_m.old_id = i."ID_MATRICULA"
  LEFT JOIN demo_id_map hor_m ON hor_m.entity = 'horario' AND hor_m.old_id = i."ID_HORARIO"
  LEFT JOIN demo_id_map alum_m ON alum_m.entity = 'alumno' AND alum_m.old_id = i."ID_ALUMNO"
  LEFT JOIN demo_id_map ses_m ON ses_m.entity = 'sesion' AND ses_m.old_id = i."ID_SESION"
  LEFT JOIN demo_id_map pro_m ON pro_m.entity = 'profesor' AND pro_m.old_id = i."ID_PROFESOR"
  LEFT JOIN demo_id_map esp_m ON esp_m.entity = 'especialidad' AND esp_m.old_id = i."ID_ESPECIALIDAD"
  LEFT JOIN demo_id_map cur_m ON cur_m.entity = 'curso' AND cur_m.old_id = i."ID_CURSO"
  LEFT JOIN demo_id_map aul_m ON aul_m.entity = 'aula' AND aul_m.old_id = i."ID_AULA"
  WHERE i."ID_CLIENTE" = p_source_cliente AND i."ID_CENTRO" = p_source_centro;

  GET DIAGNOSTICS v_n = ROW_COUNT;
  v_counts := v_counts || jsonb_build_object('INCIDENCIAS', v_n);

  INSERT INTO demo_id_map (entity, old_id, new_id)
  SELECT 'lead', l."ID_LEAD", public.demo_gen_prefixed('LED')
  FROM public."LEADS" l
  WHERE l."ID_CLIENTE" = p_source_cliente AND l."ID_CENTRO" = p_source_centro;

  INSERT INTO public."LEADS" (
    "ID_LEAD", "FECHA", "ID_CLIENTE", "NOMBRE", "NOMBRE_CONTACTO", "TELEFONO", "ID_PROFESOR",
    "ESPECIALIDAD", "DIA", "HORA_INICIO", "HORA_FIN", "ESTADO", "RESUMEN", "ID_AULA",
    "CLASE_REALIZADA", "ID_CENTRO", "created_at", "updated_at", "ID_CURSO", "EMAIL_LEAD"
  )
  SELECT
    lm.new_id,
    l."FECHA",
    ...
    l."EMAIL_LEAD"
  FROM public."LEADS" l
  JOIN demo_id_map lm ON lm.entity = 'lead' AND lm.old_id = l."ID_LEAD"
  LEFT JOIN demo_id_map pm ON pm.entity = 'profesor' AND pm.old_id = l."ID_PROFESOR"
  LEFT JOIN demo_id_map am ON am.entity = 'aula' AND am.old_id = l."ID_AULA"
  LEFT JOIN demo_id_map cm ON cm.entity = 'curso' AND cm.old_id = l."ID_CURSO"
  WHERE l."ID_CLIENTE" = p_source_cliente AND l."ID_CENTRO" = p_source_centro;
  ...
  INSERT INTO demo_id_map (entity, old_id, new_id)
  SELECT 'mandato', ms."ID_MANDATO", public.demo_gen_prefixed('MAN')
  FROM public."MANDATOS_SEPA" ms
  WHERE ms."ID_CLIENTE" = p_source_cliente AND ms."ID_CENTRO" = p_source_centro;

  INSERT INTO public."MANDATOS_SEPA" (
    "ID_MANDATO", "ID_ALUMNO", "ID_CLIENTE", "ID_CENTRO", "TOKEN_PUBLICO", "ESTADO", "FIRMADO_AT",
    "IP_DIRECCION", "USER_AGENT", "HASH_EVIDENCIA", "PDF_URL", "created_at"
  )
  SELECT
    mm.new_id,
    am.new_id,
    ...
    ms."created_at"
  FROM public."MANDATOS_SEPA" ms
  JOIN demo_id_map mm ON mm.entity = 'mandato' AND mm.old_id = ms."ID_MANDATO"
  LEFT JOIN demo_id_map am ON am.entity = 'alumno' AND am.old_id = ms."ID_ALUMNO"
  WHERE ms."ID_CLIENTE" = p_source_cliente AND ms."ID_CENTRO" = p_source_centro;

  GET DIAGNOSTICS v_n = ROW_COUNT;
  v_counts := v_counts || jsonb_build_object('MANDATOS_SEPA', v_n);
  ...
  INSERT INTO demo_id_map (entity, old_id, new_id)
  SELECT 'permiso', ap."ID_PERMISO", public.demo_gen_prefixed('AUS')
  FROM public."AUSENCIAS_PERMISOS" ap
  WHERE ap."ID_CLIENTE" = p_source_cliente AND ap."ID_CENTRO" = p_source_centro;

  INSERT INTO public."AUSENCIAS_PERMISOS" (
    "ID_CLIENTE", "ID_PROFESOR", "ID_PERMISO", "TIPO", "FECHA_INICIO", "FECHA_FIN", "ESTADO",
    "JUSTIFICANTE", "GASTO_VACACIONES", "GASTO_AP", "ID_CENTRO", "ID_CURSO"
  )
  SELECT
    p_target_cliente,
    pm.new_id,
    ...
    cm.new_id
  FROM public."AUSENCIAS_PERMISOS" ap
  JOIN demo_id_map am ON am.entity = 'permiso' AND am.old_id = ap."ID_PERMISO"
  LEFT JOIN demo_id_map pm ON pm.entity = 'profesor' AND pm.old_id = ap."ID_PROFESOR"
  LEFT JOIN demo_id_map cm ON cm.entity = 'curso' AND cm.old_id = ap."ID_CURSO"
  WHERE ap."ID_CLIENTE" = p_source_cliente AND ap."ID_CENTRO" = p_source_centro;

  GET DIAGNOSTICS v_n = ROW_COUNT;
  v_counts := v_counts || jsonb_build_object('AUSENCIAS_PERMISOS', v_n);

  INSERT INTO demo_id_map (entity, old_id, new_id)
  SELECT 'evaluacion', e."ID_EVALUACION", public.demo_gen_prefixed('EVA')
  FROM public."EVALUACIONES" e
  WHERE e."ID_CLIENTE" = p_source_cliente AND e."ID_CENTRO" = p_source_centro;

  INSERT INTO public."EVALUACIONES" (
    "ID_EVALUACION", "ID_CLIENTE", "ID_PROFESOR", "TRIMESTRE", "ID_CURSO", "ID_ALUMNO",
    "ID_ESPECIALIDAD", "NOTA_MEDIA", "COMENTARIOS", "ID_RUBRICA", "RESULTADOS_RUBRICA",
    "CREADO_POR", "MODIFICADO_POR", "CREATED_AT", "UPDATED_AT", "ID_CENTRO", "ESTADO", "ID_GRUPO"
  )
  SELECT
    em.new_id,
    p_target_cliente,
    ...
    gm.new_id
  FROM public."EVALUACIONES" e
  JOIN demo_id_map em ON em.entity = 'evaluacion' AND em.old_id = e."ID_EVALUACION"
  LEFT JOIN demo_id_map pm ON pm.entity = 'profesor' AND pm.old_id = e."ID_PROFESOR"
  LEFT JOIN demo_id_map cm ON cm.entity = 'curso' AND cm.old_id = e."ID_CURSO"
  LEFT JOIN demo_id_map am ON am.entity = 'alumno' AND am.old_id = e."ID_ALUMNO"
  LEFT JOIN demo_id_map esp_m ON esp_m.entity = 'especialidad' AND esp_m.old_id = e."ID_ESPECIALIDAD"
  LEFT JOIN demo_id_map rm ON rm.entity = 'rubrica' AND rm.old_id = e."ID_RUBRICA"
  LEFT JOIN demo_id_map gm ON gm.entity = 'grupo' AND gm.old_id = e."ID_GRUPO"
  WHERE e."ID_CLIENTE" = p_source_cliente AND e."ID_CENTRO" = p_source_centro;

  GET DIAGNOSTICS v_n = ROW_COUNT;
  v_counts := v_counts || jsonb_build_object('EVALUACIONES', v_n);
...
BEGIN
  ...
  SELECT p."ID_PERFIL", p."ID_CLIENTE", p."ID_CENTRO"
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO public."CENTROS" (
      "ID_CENTRO", "ID_CLIENTE", "NOMBRE_CENTRO", "ESTADO"
    ) VALUES (
      v_centro_id, v_demo_id, 'Madrid', 'ACTIVO'
    );
  END IF;
...
REVOKE ALL ON FUNCTION public.next_demo_cliente_id() FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.next_demo_cliente_id() TO service_role;
