-- Dashboard live: corregir buckets de profesores según tabla de reglas.
-- en_clase requiere sesión + último movimiento Entrada|Fin de Pausa (no Inicio Pausa).
-- alerta_grave: sin sesión + último movimiento Entrada|Inicio Pausa|Fin de Pausa.
-- ultimo_fichaje_reloj: excluir ESTADO='Anulado por Corrección' (mismo criterio que sys_alertas_fichaje_sesiones).
...
BEGIN
    ...
    clasificados AS (
        SELECT
            ...
            ufr.last_movement,
            coalesce(
                ufr.last_movement IN ('Entrada', 'Inicio Pausa', 'Fin de Pausa'),
                false
            ) AS actively_clocked_in
    ...
    bucketed AS (
        SELECT
            ...
            CASE
                ...
                WHEN c.has_active_session THEN 'ocupados_sin_fichar'
                WHEN NOT c.has_active_session AND c.actively_clocked_in THEN 'alerta_grave'
                ELSE 'libres'
            END AS bucket
        FROM clasificados c
    )
