-- Dashboard live: clasificación de profesores (sesión activa × último fichaje reloj hoy).
...
BEGIN
    ...
    bucketed AS (
        SELECT
            ...
            CASE
                WHEN c.has_active_session AND c.actively_clocked_in THEN 'en_clase'
                WHEN c.has_active_session AND NOT c.actively_clocked_in THEN 'ocupados_sin_fichar'
                WHEN NOT c.has_active_session AND NOT c.actively_clocked_in THEN 'libres'
                ELSE 'alerta_grave'
            END AS bucket
        FROM clasificados c
    )
