-- D3: reloj demo 7 días (Europe/Madrid vía timestamptz + interval).

ALTER TABLE public."CLIENTES"
  ADD COLUMN IF NOT EXISTS "DEMO_TRIAL_STARTED_AT" timestamptz NULL;

ALTER TABLE public."CLIENTES" DISABLE TRIGGER tr_validar_datos_empresa;

UPDATE public."CLIENTES" c
SET
  "CIF" = COALESCE(c."CIF", esc."CIF"),
  "DIRECCION" = COALESCE(c."DIRECCION", esc."DIRECCION"),
  "URL_WEB" = COALESCE(c."URL_WEB", esc."URL_WEB"),
  "APP_LOGO" = COALESCE(c."APP_LOGO", esc."APP_LOGO")
FROM public."CLIENTES" esc
WHERE esc."ID_CLIENTE" = 'ESC_018'
  AND c."ID_CLIENTE" LIKE 'DEMO-%'
  AND (
    c."CIF" IS NULL
    OR c."DIRECCION" IS NULL
    OR c."URL_WEB" IS NULL
    OR c."APP_LOGO" IS NULL
  );

UPDATE public."CLIENTES" c
SET "DEMO_TRIAL_STARTED_AT" = COALESCE(
  (SELECT MIN(p.created_at) FROM public."PERFILES" p WHERE p."ID_CLIENTE" = c."ID_CLIENTE"),
  now()
)
WHERE c."ID_CLIENTE" LIKE 'DEMO-%'
  AND c."DEMO_TRIAL_STARTED_AT" IS NULL;

ALTER TABLE public."CLIENTES" ENABLE TRIGGER tr_validar_datos_empresa;

CREATE OR REPLACE FUNCTION public.enforce_demo_trial(p_id_cliente text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF p_id_cliente IS NULL OR p_id_cliente NOT LIKE 'DEMO-%' THEN
    RETURN jsonb_build_object('expired', false, 'skipped', true);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Sin acceso al tenant demo';
  END IF;

  SELECT c."ESTADO_CLIENTE", c."DEMO_CAL_MEETING_AT", c."DEMO_TRIAL_STARTED_AT"
  INTO v_estado, v_meeting, v_started
  FROM public."CLIENTES" c
  WHERE c."ID_CLIENTE" = p_id_cliente;

  IF NOT FOUND THEN
    RETURN jsonb_build_object('expired', false, 'error', 'cliente_no_encontrado');
  END IF;
  ...
  IF v_meeting IS NOT NULL THEN
    ...
    RETURN jsonb_build_object(
      'expired', false,
      'has_meeting', true,
      'estado_cliente', v_estado
    );
  END IF;

  IF v_started IS NOT NULL
     AND timezone('Europe/Madrid', now()) >= timezone('Europe/Madrid', v_started) + interval '7 days' THEN
    ALTER TABLE public."CLIENTES" DISABLE TRIGGER tr_validar_datos_empresa;
    UPDATE public."CLIENTES"
    SET "ESTADO_CLIENTE" = 'Suspendido'
    WHERE "ID_CLIENTE" = p_id_cliente
      AND "ESTADO_CLIENTE" IS DISTINCT FROM 'Suspendido';
    ALTER TABLE public."CLIENTES" ENABLE TRIGGER tr_validar_datos_empresa;
  ...
  RETURN jsonb_build_object(
    'expired', v_expired OR v_estado = 'Suspendido',
    'has_meeting', false,
    'estado_cliente', v_estado
  );
END;
...
REVOKE ALL ON FUNCTION public.enforce_demo_trial(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.enforce_demo_trial(text) TO authenticated;

-- Solo INSERT: fijar inicio de prueba en altas nuevas (UPDATE de reintento no lo pisa).
